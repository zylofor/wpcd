...
class PlotUtils:
    def __init__(self, hparams, logger=None, samples=None):
        ...
        self.pca_1 = PCA(n_components=2)
        self.pca_2 = PCA(n_components=1)
    ...
    def plot_decision_regions(
        ...
        # xx, yy,x_3d,x_4d,x_5d = np.meshgrid(*arrays_for_meshgrid)
        ...
        # xx = np.squeeze(np.squeeze(xx[:,:,0,0])[:,:,0])
        # yy = np.squeeze(np.squeeze(yy[:,:,0,0])[:,:,0])
        # yhat = yhat[:len(xx.reshape(-1)),:]
